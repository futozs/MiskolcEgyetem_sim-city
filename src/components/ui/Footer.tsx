...
export default function Footer() {
  ...
  // Show hint exactly every 5 seconds
  useEffect(() => {
    // Don't show hints if the Easter egg has been activated
    ...
    const showHintInterval = setInterval(() => {
      if (!showEasterEgg) {
        ...
        // Hide hint after exactly 5 seconds
        setTimeout(() => {
          setShowHint(false);
        }, 5000);
      }
    }, 10000); // Show hint exactly every 10 seconds (5 on, 5 off)
    
    // Initial hint show
    if (!showEasterEgg && !easterEggActivated) {
      setShowHint(true);
      setTimeout(() => {
        setShowHint(false);
      }, 5000);
  ...
  }, [showEasterEgg, easterEggActivated]);
  ...
  const handleLogoClick = () => {
    ...
    if (clickCount === 1) { // 2 clicks needed
      ...
      setClickCount(0);
      setAutoHearts(true);
      setEasterEggActivated(true); // Mark that Easter egg has been activated
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6">
          ...
          >
            ...
            <AnimatePresence>
              {showHint && (
                <motion.div
                  className={`absolute -left-40 top-0 px-2 py-1 text-xs rounded-full ${
