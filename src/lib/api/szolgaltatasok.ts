...
const REFRESH_INTERVAL = 20000; // 20 seconds refresh interval for services
...
export async function fetchSzolgaltatasok(): Promise<SzolgaltatasokResponse> {
  try {
    // Használjuk a cache-elt fetch-et 20 másodperces cache idővel
    return await fetchWithRetry<SzolgaltatasokResponse>(ENDPOINT, 10, 500, REFRESH_INTERVAL);
  } catch (error) {
